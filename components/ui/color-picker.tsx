...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    const positionY =
      topLightness > 0
        ? Math.max(0, Math.min(1, 1 - lightness / topLightness))
        : 0
...
export const ColorPickerHue = ({
  ...
  return (
    <Slider.Root
      className={cn(
        'relative flex h-4 w-full touch-none select-none',
        className
      )}
      max={360}
      step={1}
    ...
    >
      <Slider.Control className="flex w-full touch-none items-center py-1">
        ...
        <Slider.Thumb
          index={0}
          aria-label="Hue"
          className="block h-4 w-4 rounded-full border-2 border-white bg-white shadow-md transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50"
        />
      </Slider.Control>
    </Slider.Root>
  )
...
export const ColorPickerAlpha = ({
  ...
  return (
    ...
    >
      <Slider.Control className="flex w-full touch-none items-center py-1">
        <Slider.Track className="relative h-3 w-full grow overflow-hidden rounded-full bg-[url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAFklEQVQYV2P8z8DwnwEPYBxVMK6iAABT1QbYBaAipgAAAABJRU5ErkJggg==)] bg-[length:8px_8px] bg-center">
          <div
...
export const ColorPickerFormat = ({
  ...
  return (
    <div
      ...
      className={cn(
        'flex h-8 overflow-hidden rounded-md ring-1 ring-border ring-inset',
        className
      )}
      {...props}
    >
      {FORMATS.map((format) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex-1 px-1.5 text-[0.65rem] font-medium tracking-wider uppercase transition-colors',
              active
...
export const ColorPickerOutput = ({
  ...
  const { primary, suffix } = useMemo(
    () => formatValue(mode, color),
    [mode, color]
  )
  ...
  return (
    <div
      className={cn(
        'flex h-8 w-full items-center gap-1.5 rounded-md bg-secondary px-2',
        className
      )}
      {...props}
    >
      ...
      <Input
        ...
        className="h-6 flex-1 border-0 bg-transparent px-1 font-mono text-[0.7rem] tracking-tight uppercase shadow-none focus-visible:ring-0"
      />
      {suffix ? (
